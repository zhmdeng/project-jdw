...
if __name__ == '__main__':
    ...
    try:
        ...
        order = f"""
                select * from wms.order_skus where created !='0000-00-00'
                """
        ...
        # tmall.columns = [['子订单编号','主订单编号','标题','价格','购买数量','外部系统编号','商品属性','套餐信息','备注','订单状态',
        #                  '商家编码','支付单号','买家应付货款','买家实际支付金额','退款状态','退款金额','订单创建时间','订单付款时间','title']]

        # columns = []
        # for i in range(len(tmall.columns)-1):
        #     columns.append(chr(tmall.columns[i]))
        #
        # tmall.columns = columns
        ...
        # tmall['info'] = tmall['info'].fillna(value='unknow')
        # tmall['postscript'] = tmall['postscript'].fillna(value='unknow')
        # tmall['product_attribut'] = tmall['product_attribut'].fillna(value='unknow')
        # tmall['outer_code'] = tmall['outer_code'].fillna(value='000000')
        # tmall['code'] = tmall['code'].fillna(value='000000')
        # tmall['sku_name'] = tmall['sku_name'].fillna(value='000000')
        # tmall['price'] = tmall['price'].fillna(value=-1.0)

        values = ['000000','000000','000000',-1.00,0,'000000','unknow','unknow','unknow',
                  'unknow','000000','000000',-1.00,-1.00,'unknow',-1.00,'0000-00-00 00:00:00','0000-00-00 00:00:00','unknow']
        base.fillna_col(tmall,tmall.columns,values,log_file)

        tmall['port'] = '天猫'
        logs.info("", f"******************* tmall data load successfully! data size:{tmall.info} *******************", log_file)
        ...
        douy['port'] = '抖音'
        final_data = douy
        logs.info("", f"******************* douy data load successfully! data size:{douy.info} *******************", log_file)
        ...
        kshou.columns = ['outer_code','code','oid','order_id','created','pay_time','order_status','order_total_fee','num','postscript','refund_status',
                         'sku_name','product_attribut','price','title','product_id']

        values = ['000000','000000','000000','000000','0000-00-00 00:00:00','0000-00-00 00:00:00','unknow',-1.00,0,'unknow','unknow',
        ...
        logs.info("", f"******************* kshou data load successfully! data size:{kshou.info} *******************", log_file)
        ...
        private.columns = ['oid','order_id','order_status','created','sku_name','product_attribut','outer_code','code',
                           'price','num','order_total_fee','postscript','refund_status','refund_money','port','pay_code','comments']
        ...
        for i in range(len(private)):
            ...
            if "Doctor’s Best" in str(private['sku_name'][i]) or ("Doctor's" in str(private['sku_name'][i])) or ("DRB" in str(private['sku_name'][i])) or ("DRB" in str(private['outer_code'][i])) or ("DRB" in str(private['product_attribut'][i])):
                private['title'][i] = "Doctor's Best"
            elif 'Zipfizz' in str(private['sku_name'][i]) or ("zip" in str(private['outer_code'][i])) or ("zip" in str(private['product_attribut'][i])):
        ...
        logs.info("", f"******************* private data load successfully! data size:{private.info} *******************", log_file)
        ...
        final_data.drop_duplicates(['oid', 'order_id', 'order_status', 'created', 'sku_name','product_attribut', 'outer_code',
        ...
        logs.info("", "data load to excel successful ...........",log_file)
        print(final_data.columns)
        col_name = ['oid', 'order_id', 'sku_name', 'price', 'num', 'outer_code','product_attribut', 'info', 'postscript', 'order_status',
                  'code','pay_code', 'order_total_fee', 'order_payment', 'refund_status','refund_money', 'created', 'title','port',
                  'product_id','comments','load_date', 'update_date']

        values = ['000000', '000000', '000000', -1.00, 0, '000000', 'unknow', 'unknow', 'unknow', 'unknow',
                  '000000', '000000', -1.00, -1.00, 'unknow', '-1.00',  '1970-01-01 01:01:01', '1970-01-01 01:01:01','unknow', 'unknow',
